...
//            List<WebElement> tabs = headerTab.findElements(By.tagName("li"));
        ...
        for (int i = 0; i < header_tabs.size(); i++) {
            ...
            switch (i) {
                case 0:
                    try {
                        wait.waitUntilVisible(By.cssSelector("#allDepartmentsFlyout"));
                        areVisible[i] = true;
                    } catch (Exception e) {
                        areVisible[i] = false;
                    }
                ...
                case 1:
                    try {
                        library.wait.waitUntilVisible(By.cssSelector(".homeDecorFurnitureFlyout.homeDecorFurnitureFlyout__menu-wrapper.homeDecorFurnitureFlyout--open"));
                        areVisible[i] = true;
                    } catch (Exception e) {
                        areVisible[i] = false;
                    }
                ...
                case 2:
                    try {
                       wait.waitUntilVisible(By.xpath("(//section[@class='grid diyFlyout__level1'])[1]"));
                        areVisible[i] = true;
                    } catch (Exception e) {
                        areVisible[i] = false;
                    }
                ...
                case 3:
                    try {
                     wait.waitUntilVisible(By.xpath("(//section[@class='grid diyFlyout__level1'])[2]"));
                        areVisible[i] = true;
                    } catch (Exception e) {
                        areVisible[i] = false;
                    }
                ...
                case 4:
                    try {
                      wait.waitUntilVisible(By.cssSelector(".grid.specialOffersFlyout__level1"));
                        areVisible[i] = true;
                    } catch (Exception e) {
                        areVisible[i] = false;
                    }
